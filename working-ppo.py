...
class Actor(nn.Module):
    def __init__(self):
        ...
        self.scheduler = optim.lr_scheduler.StepLR(self.optimizer, step_size=500, gamma=0.90)
        self.to(DEVICE)
...
class Agent:
    ...
    def choose_action(self, observation):
        ...
        min_action = torch.tensor(env.action_space.low, dtype=torch.float).to(DEVICE)
        max_action = torch.tensor(env.action_space.high, dtype=torch.float).to(DEVICE)
